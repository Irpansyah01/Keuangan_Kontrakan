...
st.set_page_config(page_title="NEXUS - Keuangan Kontrakan", layout="wide")

st.title("🏠 NEXUS - Keuangan Kontrakan")
st.write("Kelola keuangan bersama penghuni kontrakan secara praktis dan adil.")
st.caption("Datamu sekarang tersinkronisasi aman dengan Google Sheets cloud.")
st.divider()
...
if data_mentah:
    ...
    for r in data_mentah:
        id_r, tgl, brg, nom, oleh = r[0], r[1], r[2], r[3], r[4]
        brg_str = str(brg)
        brg_bersih = brg_str.split(" | Ikut: ")[0] if " | Ikut: " in brg_str else brg_str
        clean_rows.append([id_r, tgl, brg_bersih, nom, oleh, brg_str])
    df = pd.DataFrame(clean_rows, columns=["id", "tanggal", "barang", "nominal", "dibayar_oleh", "barang_mentah"])
else:
    df = pd.DataFrame(columns=["id", "tanggal", "barang", "nominal", "dibayar_oleh", "barang_mentah"])

# 1. MENU RINGKASAN
if st.session_state["menu_aktif"] == "RINGKASAN":
    st.header("📊 Ringkasan Statistik")
    if df.empty:
        st.info("Belum ada data pengeluaran. Jika kamu baru menghubungkan Google Sheet, silakan coba input nota pertama di menu TAMBAH.")
    else:
    ...
    with st.form("form_kas_baru", clear_on_submit=True):
        ...
        if st.form_submit_button("💾 Simpan Data"):
            if barang.strip() == "" or nominal <= 0 or len(siapa_ikut) == 0:
                ...
                st.success("Berhasil dicatat di memori aman! Silakan buka Google Sheets kamu untuk melihat pembaruan data.")
                st.rerun()

# 3. MENU PEMBAYARAN MINGGUAN
    ...
    if df.empty:
        ...
        for _, row in df.iterrows():
            nama_barang_mentah = str(row["barang_mentah"])
        ...
        if hari_ini != 6:  # Kunci selain Hari Minggu
            st.warning("⚠️ Pembayaran hanya dibuka setiap hari **Minggu**. Hutang otomatis diakumulasikan ke minggu depan.")
...
# 4. MENU RIWAYAT
    ...
    if df.empty:
        ...
        with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
            df[["tanggal", "barang", "nominal", "dibayar_oleh"]].to_excel(writer, sheet_name='Data_Kas', index=False)
        ...
        st.subheader("Riwayat Nota (Tersinkronisasi Cloud):")
        st.dataframe(df[["id", "tanggal", "barang", "nominal", "dibayar_oleh"]], use_container_width=True, hide_index=True)
        
        st.divider()
        st.subheader("🗑️ Hapus Data")
        id_target = st.number_input("Masukkan ID Nota untuk dihapus:", min_value=0, step=1)
        if st.button("❌ Hapus", type="primary") and id_target > 0:
